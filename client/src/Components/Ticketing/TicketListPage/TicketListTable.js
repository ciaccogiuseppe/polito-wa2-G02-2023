        ...
        <div style={{alignItems:"center", alignSelf:"center"}}>
            <table className="table  roundedTable"  style={{alignContent: "center", width: "70%", margin: "auto", marginTop:"20px"}}>
                ...
                <tr className="text-light">
                    <th><h5>TITLE</h5></th>
                    <th width={"15%"}><h5>PRODUCT</h5></th>
                    <th width={"15%"}><h5>STATUS</h5></th>
                    <th width={"10px"}><h5>PRIORITY</h5></th>
                    <th width={"15%"}><h5>CREATED</h5></th>
                </tr>
                </thead>
                <tbody>

                {ticketList.map(t => <TicketTableTR
                    title={t.title}
                    product={t.product}
                    priority={t.priority}
                    status={t.status}
                    id={t.ticketId}
                    createdTime={
                        new Date(t.createdTimestamp).getDate().toString().padStart(2,"0")+ "/" +
                        (new Date(t.createdTimestamp).getMonth()+1).toString().padStart(2,"0")+ "/" +
